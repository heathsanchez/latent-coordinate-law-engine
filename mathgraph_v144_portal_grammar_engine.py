#!/usr/bin/env python3
"""MATHGRAPH v144: Portal Grammar Discovery Engine.

Single-file, Colab-ready research engine for testing the hypothesis:

    Coordinates may not transfer. Coordinate grammars may transfer.

The script searches for sparse expression coordinates, parses successful
coordinates into operator grammars, ranks operator families, evaluates family
transfer, evolves failed operators from residuals, and exports a measured
lawbook. It intentionally reports weak evidence as weak evidence.
"""
...
def write_reports(
    ...
    write_markdown_report(out, portal_df, representation_df, family_rankings, lawbook, metrics, manifest)
...
def write_markdown_report(
    ...
    lines = [
        "# MATHGRAPH v144 Portal Grammar Discovery Report",
        "",
        "Hypothesis: coordinates may not transfer, but coordinate grammars may transfer.",
        "",
        "## Run Manifest",
        "",
    ...
    lines.extend(["", "## 1. Which Operators Survive?", ""])
    if family_rankings.empty:
    ...
    lines.extend(["", "## 4. Are There Universal Coordinate Grammars?", ""])
    if lawbook["laws"]:
        lines.append("Reusable operator grammars were measured, but universality is not claimed.")
    else:
        lines.append("No operator grammar met the promotion rule for cross-domain reuse.")
    lines.extend(["", "## 5. Is Representation Search Actually Operator Search?", ""])
    lines.append(
        "This run measures that claim indirectly: coordinate successes are aggregated into operator-family survival, "
        "transfer, and residual-evolution evidence."
    )
    ...
    lines.extend(["", "## Lawbook", "", f"Promoted grammar laws: {len(lawbook['laws'])}", ""])
    for law in lawbook["laws"][:20]:
        lines.append(
            f"- {law['law']}: domains={law['domains']}, score={law['score']:.3f}, "
            f"transfer={law['transfer_rate']:.3f}"
        )
...
def verdict(metrics: dict[str, float], lawbook: dict[str, Any]) -> tuple[str, str]:
    ...
    surviving_families = metrics["surviving_operator_families"]
    best_score = metrics["best_operator_grammar_score"]
    if law_count >= 3 and transfer_coverage >= 0.35 and best_score > 1.0:
        return "A", "strong portal grammar discovered"
    if law_count >= 1 or surviving_families >= 2:
        return "B", "weak but measurable operator transfer"
    return "C", "no evidence for reusable portal operators"
...
This is a research result, not a proof. A grammar law is promoted only when an
operator family appears in multiple domains, has measured advantage, and
survives family-level transfer checks.

Scientific caution: this run must not be read as universal portals, universal
coordinates, or intelligence solved. It only reports measured operator-family
evidence.
"""
...
def build_counterexamples(portal_df: pd.DataFrame, transfer_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not portal_df.empty:
        ...
        for _, row in failures.iterrows():
            rows.append(
                {
                    "type": "failed_operator",
                    "domain": row["domain"],
                    "coordinate": row["coordinate"],
                    "operator_family": row["operator_family"],
                    "detail": f"{row['failure_mode']}; residual={row['residual']:.3f}; try={','.join(row['evolved_candidates'])}",
                }
            )
            ...
            rows.append(
                {
                    ...
                    "coordinate": row["operator_family"],
                    "operator_family": row["operator_family"],
                    "detail": row["status"],
                }
            )
    return pd.DataFrame(rows, columns=["type", "domain", "coordinate", "operator_family", "detail"])
...
def run_engine(args: argparse.Namespace) -> dict[str, Any]:
    ...
    metrics = final_metrics(portal_df, basin_df, transfer_df, representation_df, family_rankings)
    manifest = {
        "system": "MATHGRAPH v144 Portal Grammar Discovery Engine",
        "out": str(out),
        "seed": args.seed,
        "n": args.n,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MATHGRAPH v144 Portal Grammar Discovery Engine")
    parser.add_argument("--out", default="mathgraph_v144_out", help="Output directory")
